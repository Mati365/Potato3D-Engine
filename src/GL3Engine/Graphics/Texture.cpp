...
namespace GL3Engine {
    ...
    Texture::Texture(
            const IPoint2D& _size, GLenum _type, GLenum _bytes, GLuint _flags)
            :
              flags(_flags) {
    ...
    void Texture::generate(const IPoint2D& size, GLenum type, GLenum bytes) {
        ...
        {
            glTexImage2D(GL_TEXTURE_2D, 0, type, size.X, size.Y, 0, type, bytes,
                    0);
        }
    ...
    void TextureArray::create() {
        ...
        for (GLuint i = 0; i < textures.size(); ++i) {
            _TEX tex;
            {
                tex.data = SOIL_load_image(textures[i].c_str(), &tex.size.X,
                        &tex.size.Y, &tex.mipmaps, SOIL_LOAD_RGBA);
            }
            data[i] = tex;
            if (tex.size > biggest)
                biggest = tex.size;
        ...
        for (GLuint i = 0; i < textures.size(); ++i) {
            glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, data[i].size.X,
                    data[i].size.Y, 1, GL_RGBA, GL_UNSIGNED_BYTE, data[i].data);
